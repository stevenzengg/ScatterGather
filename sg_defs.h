...
typedef struct{
    int SG_FILE_SIZE;
    uint32_t SG_FILE_HANDLE;
    float SG_FILE_POINTER;
    SG_STATUS SG_FILE_STATUS;  
    char* SG_FILE_PATHNAME;
    SG_Node_ID* SG_FILE_NODES; 
    SG_Block_ID* SG_FILE_BLOCKS;
    uint32_t maxBlocks;
} SG_FILE;
...
typedef struct{
    uint64_t CNODE;   // The type for node identifiers
    uint64_t CBLOCK; 
    char CDATA[1024];
    uint64_t CTIME;
}SG_CACHE;
